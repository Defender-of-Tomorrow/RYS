...
def preprocessing(no):  # 검출 전처리
    image = cv.imread('images/woman/%02d.jpg' %no, cv.IMREAD_COLOR)
    if image is None: return None, None
    gray = cv.cvtColor(image, cv.COLOR_BGR2GRAY)  # 명암도 영상 변환
...
def calc_rotMat(center, pts):
    ...
    angle = cv.fastAtan2(dy, dx)                   # 차분으로 기울기 계산
...
def correct_image(image, rot_mat, eyes_centers):
    ...
    # # 행렬 처리 방식
    # center = cv.gemm(homo_coord, rot_mat.T, 1, None, 1)
