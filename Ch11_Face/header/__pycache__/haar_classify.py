...
def classify(image, hists, no):
    ...
    criteria1 = cv.compareHist(hists[2], hists[3], cv.HISTCMP_CORREL)     # 유사도 비교
    criteria2 = cv.compareHist(hists[0], hists[1], cv.HISTCMP_CORREL)
...
def display(image, face_center, eyes_center, sub):
    ...
    cv.circle(image, eyes_center[0], 10, (0, 255, 0), 2)	# 눈 표시
    cv.circle(image, eyes_center[1], 10, (0, 255, 0), 2)
