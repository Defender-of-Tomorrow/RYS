...
def draw_ellipse(image, sub, color, ratio, thickness=cv.FILLED):
    x, y, w, h = sub
...
def calc_histo(image, sub_roi, masks):
    ...
    hists = [cv.calcHist([sub], [0,1,2], mask, bins, ranges) for sub, mask in zip(subs, masks)]
    hists = [ h / np.sum(h) for h in hists]

    hsv = [cv.cvtColor(sub, cv.COLOR_BGR2HSV) for sub in subs]
    hue_hist1 = cv.calcHist([hsv[2]], [0], None, [90], [0, 180])  # Hue 채널 히스토그램 계산
    hue_hist2 = cv.calcHist([hsv[3]], [0], None, [90], [0, 180])  # Hue 채널 히스토그램 계산

    # hist_img = draw_histo(hists[2][:,:, 2])
    # cv.imshow("h", hist_img)
